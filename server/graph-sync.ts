...
export async function syncSessionToGraph(session: InterviewSession): Promise<void> {
  ...
  try {
    ...
    // 5. Create relationships: Participant-[:WORKS_AT]->Company
    await runWrite(
    ...
    if (analysis) {
      // Session-[:SCORED {score}]->ScoringDimension
      ...
      // Session-[:CLASSIFIED_AS]->Archetype
      ...
      // Session-[:FLAGGED]->RedFlag
      if (analysis.redFlags?.length) {
        for (const flag of analysis.redFlags) {
          await runWrite(
            `MATCH (s:Session {id: $sessionId})
             CREATE (rf:RedFlag {description: $description, severity: $severity, type: $type, createdAt: datetime()})
             CREATE (s)-[:FLAGGED]->(rf)`,
            {
              sessionId,
      ...
      // Session-[:HIGHLIGHTED]->GreenLight
      if (analysis.greenLights?.length) {
        for (const light of analysis.greenLights) {
          await runWrite(
            `MATCH (s:Session {id: $sessionId})
             CREATE (gl:GreenLight {description: $description, severity: $severity, type: $type, createdAt: datetime()})
      ...
      // Session-[:TRIGGERED]->Recommendation
