...
import { runResearch, confirmResearch } from "../lib/api";
...
export default function ResearchPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const data = await runResearch(sessionId!);
        clearTimeout(timer);
  ...
  const handleConfirm = async (confirmed: boolean) => {
    ...
    try {
      ...
      await confirmResearch(sessionId!, confirmed, Object.keys(corrections).length > 0 ? corrections as Record<string, string> : undefined);
      ...
      <div className="min-h-screen flex items-center justify-center px-6">
        <div className="text-center space-y-8 max-w-lg">
          <div className="relative w-24 h-24 mx-auto">
            ...
            <div className="absolute inset-4 rounded-full border-2 border-primary/60 animate-pulse" />
    ...
    <div className="min-h-screen px-6 py-12">
      <div className="max-w-2xl mx-auto space-y-8">
        <div className="text-center space-y-2">
          ...
          <p className="text-muted-foreground">We did some research to personalize your interview. You can edit any details below before confirming.</p>
        </div>
        ...
        {hasFindings ? (
          <>
              ...
              <div className="bg-muted border border-border rounded-2xl p-6 space-y-4">
                ...
                {editedBio !== undefined && (
                  <textarea
                    ...
                    rows={3}
                    className="w-full bg-muted border border-border rounded-lg px-3 py-2 text-foreground/80 leading-relaxed focus:outline-none focus:border-primary/40 focus:bg-muted transition-all resize-none text-sm"
                  />
                )}
                {research.personProfile.knownRoles && research.personProfile.knownRoles.length > 0 && (
                  <div className="flex flex-wrap gap-2">
                    {research.personProfile.knownRoles.map((role, i) => (
                      <span key={i} className="px-3 py-1 rounded-full text-xs bg-primary/10 text-primary border border-primary/20">{role}</span>
                    ))}
                  </div>
                )}
                {research.personProfile.notableAchievements && research.personProfile.notableAchievements.length > 0 && (
                  <div className="space-y-1">
                    <p className="text-muted-foreground text-xs uppercase tracking-wider">Notable</p>
            ...
            {research?.companyProfile && (
              <div className="bg-muted border border-border rounded-2xl p-6 space-y-4">
                <div className="flex items-center gap-3">
                  ...
                  <h3 className="text-lg font-medium text-foreground">Your Company</h3>
                </div>
                {editedCompanyDesc !== undefined && (
                  <textarea
                    ...
                    rows={3}
                    className="w-full bg-muted border border-border rounded-lg px-3 py-2 text-foreground/80 leading-relaxed focus:outline-none focus:border-emerald-500/40 focus:bg-muted transition-all resize-none text-sm"
                  />
                )}
                <div className="grid grid-cols-2 gap-4">
                  {research.companyProfile.founded && (
                    <div><p className="text-muted-foreground text-xs">Founded</p><p className="text-foreground/90 text-sm">{research.companyProfile.founded}</p></div>
                  )}
                  {research.companyProfile.funding && (
                    <div><p className="text-muted-foreground text-xs">Funding</p><p className="text-foreground/90 text-sm">{research.companyProfile.funding}</p></div>
                  )}
                </div>
                ...
                {research.companyProfile.recentNews && research.companyProfile.recentNews.length > 0 && (
                  <div className="space-y-1">
                    <p className="text-muted-foreground text-xs uppercase tracking-wider">Recent News</p>
                    <ul className="space-y-1">
            ...
            {editedInsights.length > 0 && (
              <div className="bg-muted border border-border rounded-2xl p-6 space-y-3">
                  ...
                  {editedInsights.map((insight, i) => (
                    <li key={i} className="text-muted-foreground text-sm flex items-start gap-2">
                      ...
                      <input
                        ...
                        className="flex-1 bg-transparent border-b border-transparent hover:border-border focus:border-primary/40 focus:outline-none text-muted-foreground text-sm py-0.5 transition-colors"
                      />
                      <button
