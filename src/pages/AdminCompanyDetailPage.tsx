  ...
  if (loading) {
    return (
      <div className="px-6 py-6 flex items-center justify-center h-full">
        <span className="text-muted-foreground/70">Loading company detail...</span>
      </div>
    );
  }
    ...
    <div className="px-6 py-6 space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div className="bg-muted/50 rounded-xl border border-border p-4">
          ...
          <p className="text-xs text-muted-foreground/70 mt-1">Total Sessions</p>
        </div>
        <div className="bg-muted/50 rounded-xl border border-border p-4">
          <span className="text-2xl font-bold text-foreground/90">{detail.participantCount}</span>
          <p className="text-xs text-muted-foreground/70 mt-1">Participants</p>
        </div>
        <div className="bg-muted/50 rounded-xl border border-border p-4">
          <span className={`text-2xl font-bold ${detail.averageScore ? scoreTextColor(detail.averageScore) : "text-muted-foreground/70"}`}>
            {detail.averageScore ?? "\u2014"}
          </span>
          <p className="text-xs text-muted-foreground/70 mt-1">Avg Score</p>
        </div>
        <div className="bg-muted/50 rounded-xl border border-border p-4">
          <span className="text-2xl font-bold text-foreground/90">{detail.completionRate}%</span>
          <p className="text-xs text-muted-foreground/70 mt-1">Completion Rate</p>
        </div>
      </div>
              ...
              <div className="space-y-3">
                {detail.dimensionAverages.map((dim) => (
                  <div key={dim.dimension} className="flex items-center gap-3">
                    ...
                    <span className="text-[10px] text-muted-foreground/50 w-8 text-right">{dim.count}s</span>
                  </div>
                ))}
              </div>
            </div>
              ...
              {activeIntelTab === "themes" && (
                <div className="space-y-4">
                  {(["tool", "pain_point", "goal", "capability"] as const).map((cat) => {
                    ...
                    return (
                      <div key={cat}>
                        <h4 className="text-[10px] uppercase tracking-wider text-muted-foreground/70 mb-2">
                  ...
                  {(!intel.themes || intel.themes.length === 0) && (
                    <div className="text-center py-6 text-muted-foreground/70 text-sm">No themes detected</div>
                  )}
                </div>
              )}
              ...
              {activeIntelTab === "people" && (
                <div>
                  {(!intel.participants || intel.participants.length === 0) ? (
                    <div className="text-center py-6 text-muted-foreground/70 text-sm">No participant data</div>
                  ) : (
                      ...
                      {intel.participants.map((p) => (
                        ...
                        >
                          <div className="flex items-center justify-between mb-2">
                            ...
                            <span className={`text-sm font-bold tabular-nums ${p.overallScore != null ? scoreTextColor(p.overallScore) : "text-muted-foreground/70"}`}>
                              {p.overallScore != null ? Math.round(p.overallScore) : "—"}
                            </span>
                          </div>
                          <p className="text-xs text-muted-foreground/70 truncate">{p.role}</p>
              ...
              {activeIntelTab === "recommendations" && (
                <div>
                  {(!intel.recommendations || intel.recommendations.length === 0) ? (
                    <div className="text-center py-6 text-muted-foreground/70 text-sm">No recommendations yet</div>
                  ) : (
                    <div className="space-y-3">
                          ...
                          <div key={idx} className="flex items-start gap-3 p-3 rounded-xl bg-white/[0.02] border border-border/50">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="flex items-center gap-3 mt-2">
                                <span className="text-[10px] text-muted-foreground/70">{rec.estimatedValue}</span>
                                <span className="text-[10px] text-muted-foreground/70">{rec.frequency}x triggered</span>
              ...
              {activeIntelTab === "benchmark" && (
                <div>
                  {(!intel.benchmarks || intel.benchmarks.length === 0) ? (
                    <div className="text-center py-6 text-muted-foreground/70 text-sm">No benchmark data</div>
                  ) : (
                    <div className="space-y-4">
                        ...
                        return (
                          <div key={b.dimension}>
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-[10px] text-muted-foreground/50 w-7 text-right flex-shrink-0">{Math.round(b.globalAvg)}</span>
                            </div>
