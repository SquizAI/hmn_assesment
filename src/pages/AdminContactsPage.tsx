  ...
  return (
    <div className="max-w-7xl mx-auto p-6">
      ...
      <div className="flex flex-wrap items-center gap-4 mb-6">
        ...
        <input type="text" placeholder="Search contacts..." value={searchInput} onChange={(e) => setSearchInput(e.target.value)} className="bg-muted border border-border rounded-lg px-4 py-2 text-sm text-foreground placeholder-white/30 focus:border-white/30 focus:outline-none w-64" />
        ...
        <button onClick={() => setShowAddForm(!showAddForm)} className="px-4 py-2 text-sm font-medium rounded-lg bg-gradient-to-r from-purple-600 to-blue-600 text-foreground hover:from-purple-500 hover:to-blue-500 transition-all">Add Contact</button>
      </div>
      ...
      {selectedIds.size > 0 && (
        <div className="mb-4 flex items-center gap-4 p-3 bg-blue-500/10 border border-blue-500/20 rounded-xl">
          ...
          <button onClick={handleCallSelected} disabled={calling} className="px-4 py-1.5 text-sm font-medium rounded-lg bg-gradient-to-r from-purple-600 to-blue-600 text-foreground disabled:opacity-50 transition-all">
      ...
      {showAddForm && (
        <div className="mb-6 p-6 bg-muted border border-border rounded-2xl">
          ...
          <form onSubmit={handleAddContact} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <input type="text" placeholder="Name *" required value={newContact.name} onChange={(e) => setNewContact({ ...newContact, name: e.target.value })} className="bg-muted border border-border rounded-lg px-4 py-2 text-sm text-foreground placeholder-white/30 focus:border-white/30 focus:outline-none" />
            <input type="text" placeholder="Phone *" required value={newContact.phone} onChange={(e) => setNewContact({ ...newContact, phone: e.target.value })} className="bg-muted border border-border rounded-lg px-4 py-2 text-sm text-foreground placeholder-white/30 focus:border-white/30 focus:outline-none" />
            <input type="email" placeholder="Email" value={newContact.email} onChange={(e) => setNewContact({ ...newContact, email: e.target.value })} className="bg-muted border border-border rounded-lg px-4 py-2 text-sm text-foreground placeholder-white/30 focus:border-white/30 focus:outline-none" />
            <input type="text" placeholder="Company" value={newContact.company} onChange={(e) => setNewContact({ ...newContact, company: e.target.value })} className="bg-muted border border-border rounded-lg px-4 py-2 text-sm text-foreground placeholder-white/30 focus:border-white/30 focus:outline-none" />
            <input type="text" placeholder="Role" value={newContact.role} onChange={(e) => setNewContact({ ...newContact, role: e.target.value })} className="bg-muted border border-border rounded-lg px-4 py-2 text-sm text-foreground placeholder-white/30 focus:border-white/30 focus:outline-none" />
            <input type="text" placeholder="Industry" value={newContact.industry} onChange={(e) => setNewContact({ ...newContact, industry: e.target.value })} className="bg-muted border border-border rounded-lg px-4 py-2 text-sm text-foreground placeholder-white/30 focus:border-white/30 focus:outline-none" />
            <input type="text" placeholder="Team Size" value={newContact.team_size} onChange={(e) => setNewContact({ ...newContact, team_size: e.target.value })} className="bg-muted border border-border rounded-lg px-4 py-2 text-sm text-foreground placeholder-white/30 focus:border-white/30 focus:outline-none" />
            <div className="flex gap-2">
              <button type="submit" className="flex-1 px-4 py-2 text-sm font-medium rounded-lg bg-gradient-to-r from-purple-600 to-blue-600 text-foreground transition-all">Add</button>
