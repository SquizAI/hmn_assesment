          ...
          <div key={opt.value}>
            <button onClick={() => handleSelect(opt.value)}
              className={`w-full text-left p-4 rounded-xl border transition-all duration-200 group
                ${active ? "bg-foreground/15 border-foreground/40 shadow-lg shadow-white/5" : "bg-muted border-border hover:bg-muted hover:border-border"}`}>
            ...
            {isOther && active && (
              <input
                ...
                autoFocus
                className="mt-2 w-full bg-muted border border-border rounded-xl px-4 py-3 text-foreground text-sm placeholder:text-muted-foreground focus:outline-none focus:border-foreground/40 focus:bg-foreground/[0.08] transition-all"
              />
            )}
          </div>
        );
