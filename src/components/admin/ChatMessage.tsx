              ...
              <tbody>
                {dataRows.map((row, i) => {
                  ...
                  return (
                    ...
                    >
                      {row.map((cell, j) => (
                        <td key={j} className={`px-3 py-2 border-b border-border ${question ? "text-foreground/80 group-hover:text-foreground/90" : "text-muted-foreground"}`}>
                          {cell.trim()}
                        </td>
                      ))}
                      {hasQuestions && (
    ...
    const flushCode = () => {
      elements.push(
        <div key={`code-${elements.length}`} className="my-3 rounded-lg overflow-hidden">
          <div className="bg-muted px-3 py-1.5 text-[10px] text-muted-foreground uppercase tracking-widest border-b border-border">
            {codeLang || "code"}
          </div>
          <pre className="bg-muted px-4 py-3 overflow-x-auto text-sm">
          ...
          <>
            ...
            {toolCalls && toolCalls.length > 0 && (
              <button
                ...
                className="flex items-center gap-2 text-[11px] text-muted-foreground hover:text-muted-foreground transition-colors py-1 px-1 group"
              >
                <svg
            ...
            {toolsExpanded && toolCalls && (
              <div className="bg-muted rounded-xl border border-border px-3 py-2 space-y-1 mb-1">
                {toolCalls.map((tc, i) => (
            ...
            <div className="bg-muted rounded-2xl rounded-bl-md px-4 py-3 text-sm leading-relaxed text-foreground/90 border border-border">
              {renderContent(body)}
            </div>
